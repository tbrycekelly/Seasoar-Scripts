...
%function data=avgall(pathstr,prefix,suffix1,suffix2,cfg) calculates
%1-second averages of ctd and engineering data from files defined by
%directory path pathstr, prefix prefix, between hex suffix1 and suffix2.
%Requires calibration structure cfg.
...
for n=hex2dec(suffix1):hex2dec(suffix2)
   ...
   elseif ~isempty(d.lat)
      ...
      mc=[mcold; d.modcount];
      ii=find(~(diff(mc) == 1 | (mc(1:end-1) == 255 & mc(2:end) == 0)));
      if ~isempty(ii)
